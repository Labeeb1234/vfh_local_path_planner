...
// 1D-Polar Histogram supporting header
...
const int GRID_LENGTH_Z = 360 / ALPHA_RES; // 0-59 bins of z

class Histogram {
  int resolution_;
  int z_dim_;
  Eigen::VectorXf dist_;

  /**
  * @brief     wraps elevation and azimuth indeces around the histogram
  * @param     i, binned azimuth angle index
  **/
  inline void wrapIndex(int& i) const {
    i = i % z_dim_;
    if(i < 0){i += z_dim_;}
  }
  ...
  * @param[in] i, binned azimuth angle index
  * @returns   distance to the vehicle of obstacle mapped to (x, y) cell [m]
  **/
  inline float get_dist(int i) const {
    wrapIndex(i);
    return dist_(i);
  }
  ...
  *[m]
  **/
  inline void set_dist(int i, float value){dist_(i) = value;}
